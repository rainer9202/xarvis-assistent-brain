...
> **Note**: This file currently documents only SDD-tracked changes to the exercise domain, starting from the `add-exercise-personal-records` change (2026-07-17). The domain's other existing endpoints (create, list, get-by-id) predate SDD adoption and are not yet retroactively specified. Future SDD changes touching other endpoints will incrementally build out this spec.
