...
- [x] 5.1 Update `prisma/schema.prisma` — `Category.userId` from `String?` to `String`; keep `@@unique([name, movementType, userId])`. Also updated `CategoryEntity`/`CategoryProps` (`domain/entities/category.entity.ts`) from `userId?: string | null` to required `userId: string` to match — required by `pnpm typecheck` once the Prisma-generated `CategoryCreateInput.userId` became non-nullable (`prisma-category.repository.ts`'s `save()` no longer compiled otherwise); mirrors `AccountEntity`'s existing required-`userId` pattern.
- [x] 5.2 Ran `npx prisma generate` (clean) then `npx prisma migrate dev --create-only --name category_user_id_required` (NOT plain `pnpm db:migrate`) to produce `prisma/migrations/20260720143409_category_user_id_required/migration.sql`, then manually added the `DROP INDEX "categories_name_movement_type_global_unique"` statement (Prisma's schema-diff can't see this raw-SQL-only partial index) plus the down-migration documented as a comment (Prisma has no native down-migration file mechanism — same as this repo's existing migrations). **Deviation, documented**: did NOT run this migration against the local dev DB. `SELECT count(*) FROM categories WHERE user_id IS NULL` = 23 locally, with 44 `Movement` rows referencing them — applying `SET NOT NULL` right now would fail exactly the way it should in production without a backfill first. This is the Unit 5→Unit 6 dependency the design already calls out ("Unit 5 depends on Unit 6's backfill script having run and verified... FIRST"), and it turns out to apply locally too, not just in production. The migration will be applied+verified against the local dev DB as part of Phase 6 (6.1-6.4), once the backfill script exists to legitimately clear those 23 rows first.
- [x] 5.3 **MANUAL PRODUCTION GATE — do not automate**: documented directly inside `migration.sql` itself (travels with the file, survives PR description loss) — this migration MUST NOT be applied to production (`prisma migrate deploy`) until Phase 6's backfill script has run against production AND Phase 6's verification step confirms zero `userId: null` category rows and zero orphaned Movement references remain.
- [x] 5.4 Updated `prisma/seed.ts` — categories are now upserted per-user via `prisma.category.upsert()` on the compound `name_movementType_userId` key (real upsert, replacing the old nullable-`userId` find-then-create workaround), owned by the seeded default user.

## Phase 6: Backfill Script + Verification + E2E Coverage (Commit 6)

- [x] 6.1 RED: write a test harness (integration, real Prisma against docker-compose `db`) seeding: one existing user with zero accounts/categories/groups, one existing user with Movements referencing a global (`userId:null`) default category, and one existing user with a Movement referencing a global NON-default category (edge case)
- [x] 6.2 GREEN: implement `prisma/scripts/provision-default-user-data.ts` — per user: `upsert` own copies of the 3 default accounts/15 categories/2 groups on `(name, movementType, userId)`/equivalent natural keys; for every global category referenced by a Movement, ensure the user has an owned copy (creating a non-default one if needed, reusing that global row's own `icon` verbatim); reassign those `Movement.categoryId` to the owned copy; finally delete now-unreferenced global (`userId:null`) rows
- [x] 6.3 Assert against the Phase 6.1 harness: idempotent (running twice produces no duplicates), the zero-accounts user ends up with exactly the default set, the default-global Movement is reassigned to the user's owned default copy, and the non-default-global Movement is reassigned to a newly-created owned copy carrying the original icon
- [x] 6.4 GREEN: implement the companion dry-run/verification check (read-only script or exported function) confirming `SELECT count(*) FROM categories WHERE user_id IS NULL` = 0 and no `Movement` references an absent/global category — this is the gate Phase 5.3 depends on
- [x] 6.5 e2e: `POST /auth/sign-up` — new user ends up owning exactly 3 accounts (`Principal`/`AT02`/`isPrincipal:true`, `Ahorro`/`AT04`, `Efectivo`/`AT01`), 15 categories, 2 groups (`Casa`, `Gastos Hormigas`); `Principal` is the only `isPrincipal:true` account
- [x] 6.6 e2e: a provisioning failure (simulate via a forced repository error) leaves no `User` row behind — sign-up returns an error and a retry with the same email succeeds (proves rollback, not the rejected compensating-delete lockout)
- [x] 6.7 Run `pnpm typecheck && pnpm lint` — must be clean
- [x] 6.8 Run `pnpm test` (unit) — all green
- [x] 6.9 Run `pnpm test:e2e` against the docker-compose `db` service (`docker compose up -d db` first) — all green, including 6.5-6.6
