...
│   └── shared/             ← cross-use-case application helpers (not use-case-specific, but still application-layer — e.g. `identity/auth`'s `AuthTokenIssuer`, `movement`'s `credit-limit.ts`)
├── infrastructure/
...
**Sanctioned exception — `UpdateProfileUseCase`** (`src/modules/identity/auth/application/use-cases/update-profile.use-case.ts`): returns the full `UserProfileResponse`, not `{ id: string }`. Decided in `openspec/changes/add-user-profile-endpoints/design.md` (ADR-4), mirroring the existing `SignUpUseCase`/`SignUpResponse` precedent — `PATCH /auth/me` is a self-service profile edit where the client needs the updated profile back immediately, not a follow-up `GET`. This is the only current exception to the rule above.
...
`SignUpUseCase`, `SignInUseCase`, and `RefreshTokenUseCase` all need the identical access+refresh token-issuing step once a user is authenticated — that's extracted into the injectable `AuthTokenIssuer` (`src/modules/identity/auth/application/shared/auth-token-issuer.ts`), which signs the access JWT, signs a `{ sub, type: 'refresh', jti }` refresh JWT (the `jti` nonce guarantees uniqueness even when two tokens are issued for the same user within the same second-granularity `iat`), hashes the refresh token before persisting it via `RefreshTokenRepositoryPort`, and returns `{ id, accessToken, refreshToken }` — instead of duplicating this per use case.
